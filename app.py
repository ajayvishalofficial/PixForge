...
class CameraObject:
    ...
    def start_scan(self):
        ...
        try:
            ...
            base_timestamp = int(datetime.timestamp(datetime.now()))
            dir_name = f'photos_{base_timestamp}'
            dir_path = os.path.join("/home/pi/Desktop/Storage", dir_name)
            os.makedirs(dir_path, exist_ok=True)
            logging.info(f"Directory created successfully. Path: {dir_path}")
...
def start_scanning():
    try:
        for i in range(10):
         print("Received message - Starting")
        ser.write(b'start\n')
        cameras_data = [(camera_num, camera) for camera_num, camera in cameras.items()]
        camera = cameras.get(camera_num)
        
        camera.start_scan()  # Call your take_photo function
        time.sleep(1)
        return jsonify(success=True, message="Photo captured successfully")
    except Exception as e:
        return jsonify(success=False, message=str(e))
...
def stop_scanning():
    for i in range(10):
        print("Received message - Stopping")
    ser.write(b'stop\n')
...
def move_up():
    for i in range(10):
        print("Received message - Moving Up")
    ser.write(b'move_up\n')
    return jsonify({"message": "Stopped  successfully"}), 200
...
def move_down():
    for i in range(10):
        print("Received message - Moving Down")
